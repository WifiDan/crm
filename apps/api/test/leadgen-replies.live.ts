...
check("scratch DB has leads and sends to match against", leadCount > 900 && sendCount > 200, `${leadCount} leads, ${sendCount} sends`);
...
check("read the real mailbox (fetched > 0)", c.fetched > 0, `${c.fetched} messages`);
check(
...
check("rows written == stored", total - before === c.stored, `+${total - before}`);
check("every stored row is flagged shadow", (await db.lgInboundMessage.count({ where: { shadow: false } })) === 0);

const second = await handler.run(ctx("replies.poll"));
const total2 = await db.lgInboundMessage.count();
check("second run is idempotent (no new rows)", total2 === total, `${total} -> ${total2}`);
check("second run sees the same mailbox", (second.counters as Record<string, number>).fetched >= c.fetched);
