...
import { ReplySendService, toMailOptions } from "../src/leadgen/reply-send.service";
...
async function main() {
	...
	try {
		...
		check("reviewedBy recorded from the session", draft.reviewedBy === reviewer.email);
		...
		check("send row source crm-reply, sentBy recorded", row.source === "crm-reply" && row.sentBy === reviewer.email);
		...
		check("MIME To", mime.to && !Array.isArray(mime.to) ? mime.to.text === "alpha@example.invalid" : false);
		check("MIME From", mime.from?.text.includes("danio@elitesystemsdesign.com") ?? false, mime.from?.text);
		check("MIME Subject", mime.subject === "Re: A demo site for you");
		check("MIME Message-ID is ours", mime.messageId === r.messageId, mime.messageId);
		check("MIME In-Reply-To is the inbound id", mime.inReplyTo === `<in-${tag}-alpha@example.invalid>`, String(mime.inReplyTo));
		const refs = Array.isArray(mime.references) ? mime.references : [String(mime.references)];
		check(
			"MIME References = original send then inbound",
			refs.join(" ") === `<orig-${tag}-alpha@elitesystemsdesign.com> <in-${tag}-alpha@example.invalid>`,
			refs.join(" "),
		);
		check("MIME body is the approved text", (mime.text ?? "").trim() === "Hi there,\n\nHappy to talk it through.");
		check("row stores the same headers", row.inReplyTo === `<in-${tag}-alpha@example.invalid>` && row.referencesHeader === refs.join(" "));
		...
		const dDraft = await db.lgReplyDraft.findUniqueOrThrow({ where: { id: d.draft.id } });
		...
		check("race: exactly one fulfilled", res.filter((x) => x.status === "fulfilled").length === 1);
		check("race: exactly one mail built", raws.length === before + 1, `${raws.length - before}`);
		const rows = await db.lgOutreachSend.count({ where: { leadId: c.lead.id, step: "REPLY" } });
		...
		let ph = "";
		await service
			.sendReply({ ...input(p.draft.id, "placeholder"), body: "The price is [CHECK: price] ok?" })
			.catch((e: unknown) => {
				ph = String(e);
			});
		check("[CHECK:] placeholder refused", /placeholder/.test(ph), ph.slice(0, 80));
	} finally {
		...
		const left = await db.lgLead.count({ where: { businessName: { startsWith: tag } } });
		check("cleanup left no test rows", left === 0, String(left));
	}
